...
> **Note:** The emulator's Run key (`IsoLensEmulatorTest`) was cleaned up before autoruns ran, so it does not appear in the output. To capture it, autoruns should be run _during_ the emulator's Phase 2 execution window.
